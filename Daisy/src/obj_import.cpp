...
mesh obj_mesh_importer::get_mesh() const
{
	...
	if (m_read_texcoords)
	{
		mesh_attribute_info<vector2f> texcoords_info{ 
			TEXCOORD_ATTR_ID, m_texcoords, m_texcoords_indices, 
			attribute_interpolation_option::perspective_correct };
		m.get_vector2f_attributes().push_back(texcoords_info);
	}

	if (m_read_normals)
	{
		mesh_attribute_info<vector3f> normals_info{ 
			NORMAL_ATTR_ID, m_normals, m_normals_indices, 
			attribute_interpolation_option::perspective_correct };
...
void obj_mesh_importer::on_face_texcoord_def(
	unsigned int texcoord_index0, unsigned int texcoord_index1, unsigned int texcoord_index2)
{
	if (m_read_texcoords)
	{
...
void obj_mesh_importer::on_face_normal_def(
	unsigned int normal_index0, unsigned int normal_index1, unsigned int normal_index2)
{
	if (m_read_normals)
	{
...
mesh daisy::load_mesh_from_obj(std::string const& path, bool const read_texcoords, bool const read_normals){
